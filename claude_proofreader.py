...
- ユーザーは「直前の指示や案」への訂正を口にすることがある。「いや、その字は学校の校」「そこはひらがなで」「さっきのは取り消して」など。
  直前のやりとりが添えられている場合はそれを踏まえて解釈し、訂正後の案を出し直す。
  漢字の字指定（「学校のコウ」「にんべんの仕」など）は、その字を使った表記になるよう before/after を作る。
  音声の聞き取りでは同音異義語が別の字になりやすい（初校→初稿 など）。指示の漢字が文脈と合わないときは、音が同じで文脈に合う語として解釈してよい。
...
def proofread(manuscript, instruction, style="auto", history=None):
    """原稿と校正指示を受け取り、校正案(dict)を返す。

    Args:
        style: 文体プリセット（auto / novel / tech / biz）。
        history: 直前のやりとり [{"instruction": str, "edits": [...]}, ...]。
            「いや、その字は学校の校」のような言い直しを解釈するための文脈。

    Returns:
        {"summary": str, "edits": [{"before","after","reason","type"}, ...]}
    """
    ...
    for h in (history or [])[-3:]:
        hist_text += f"- 指示:「{h.get('instruction', '')}」\n"
        for e in (h.get("edits") or [])[:6]:
            hist_text += f"  - 案: 「{e.get('before', '')}」→「{e.get('after', '')}」\n"

    user_content = (
        (f"# 直前のやりとり（言い直し・字指定の解釈に使う）\n{hist_text}\n" if hist_text else "")
        + "# 原稿\n"
        "----\n"
        f"{manuscript}\n"
        "----\n\n"
